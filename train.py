...
def _train(model, args):
    ...
    dataloader = train_dataloader(
        args.data_dir, args.batch_size, args.num_worker)
    ...
    for epoch_idx in range(epoch, args.num_epoch + 1):
        ...
        for iter_idx, batch_data in enumerate(dataloader):
            ...
            label_img2 = F.interpolate(
                label_img, scale_factor=0.5, mode='bilinear')
            ...
            if (iter_idx + 1) % args.print_freq == 0:
                print("Time: %7.4f Epoch: %03d Iter: %4d/%4d LR: %.10f Loss content: %7.4f Loss fft: %7.4f" % (
                    iter_timer.toc(), epoch_idx, iter_idx +
                    1, max_iter, scheduler.get_lr(
                    )[0], iter_pixel_adder.average(),
        ...
        overwrite_name = os.path.join(args.model_save_dir, 'model_latest.pkl')
        ...
        if epoch_idx % args.save_freq == 0:
            save_name = os.path.join(
                args.model_save_dir, 'model_%d.pkl' % epoch_idx)
